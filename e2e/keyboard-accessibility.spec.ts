...
test.describe('T55 键盘与焦点', () => {
  ...
  test('键盘进入发布表单，校验错误与控件建立关联', async ({ page, isMobile }) => {
    ...
    await next.focus()
    await page.keyboard.press('Enter')
    await expect(page.locator('.task-error-summary')).toBeFocused()
    const definitionError = page.locator('.task-error-summary a[href="#submission-definition"]')
    await definitionError.focus()
    await page.keyboard.press('Enter')
