...
describe('personal center selector', () => {
  it('derives the registered user history from shared state without copying fixtures', () => {
    ...
    expect(data.comparisonSessions).toHaveLength(2)
    expect(data.comparisonSessions.map(({ id }) => id)).toEqual(expect.arrayContaining(['comparison-anonymous-pdf', 'comparison-mia-speaking']))
