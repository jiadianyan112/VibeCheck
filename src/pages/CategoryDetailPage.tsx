import { DiscoveryShell, DiscoveryFilters } from '../components/discovery'
import { useEffect, useMemo, useState } from 'react'
import { Link, useParams, useSearchParams } from 'react-router-dom'
import { AssetCard, Button, EmptyState, ErrorPanel, LoadingState, Tag } from '../components'
...
export function CategoryDetailPage() {
  ...
  if (!category) return <DiscoveryShell title="未找到该专题"><p>这个专题可能已调整或暂未开放。</p><Link to="/categories">返回分类总览</Link></DiscoveryShell>
  if (loading) return <DiscoveryShell title={`${category.name}专题`}><LoadingState label={`${category.name}专题加载中`} /></DiscoveryShell>
  if (error) return <DiscoveryShell title={`${category.name}专题`}><ErrorPanel message={error.message} /></DiscoveryShell>

  return (
    <DiscoveryShell title={category.shortProblem} description={category.boundary} filters={
        <DiscoveryFilters label="作品筛选"><div className="cluster cluster--between"><h2>筛选与排序</h2><Button variant="quiet" onClick={() => setParams({}, { replace: true })}>重置</Button></div>
      ...
      <section className="discovery-paths stack"><Tag tone="strong">{category.name}专题</Tag><div className="solution-paths">{category.solutionPaths.map((path, index) => <div key={path}><span>路径 {index + 1}</span><strong>{path}</strong><small>查看采用这种做法的作品</small></div>)}</div></section>

      {baseProjects[0] ? <section className="discovery-featured"><div className="section-heading"><h2>代表作品</h2></div><FeedProjectCard project={baseProjects[0]} creators={creatorsForProject(baseProjects[0])} selectedForCompare={state.comparisonProjectIds.includes(baseProjects[0].id)} onToggleCompare={(p) => state.comparisonProjectIds.includes(p.id) ? dispatch({ type: 'COMPARISON_REMOVE', projectId: p.id }) : addProject(p.id)} /></section> : null}

      <section className="discovery-results">

        <div className="stack"><div className="cluster cluster--between"><h2>作品流</h2><strong aria-live="polite">{filtered.length} 个结果</strong></div>{filtered.length ? <div className="explore-feed discovery-feed">{filtered.map((project) => <FeedProjectCard key={project.id} project={project} creators={creatorsForProject(project)} favorited={state.favoriteProjectIds.includes(project.id)} selectedForCompare={state.comparisonProjectIds.includes(project.id)} onToggleCompare={(p) => state.comparisonProjectIds.includes(p.id) ? dispatch({ type: 'COMPARISON_REMOVE', projectId: p.id }) : addProject(p.id)} onToggleFavorite={toggleFavorite} />)}</div> : <EmptyState title="没有符合筛选条件的作品" description="可以减少筛选条件，看看更多作品。" action={<Button onClick={() => setParams({}, { replace: true })}>重置筛选</Button>} />}</div>
      </section>

      <section className="stack"><div className="section-heading"><h2>最近更新</h2></div>{categoryEvents.length ? <div className="event-list">{categoryEvents.map((event) => <Link key={event.id} to={`/project/${event.projectId}#${event.id}`} className="discovery-event-link"><time dateTime={event.happenedAt}>{new Date(event.happenedAt).toLocaleDateString('zh-CN')}</time><strong>{event.summary}</strong><span>查看事件 →</span></Link>)}</div> : <EmptyState title="近期无公开事件" />}</section>
