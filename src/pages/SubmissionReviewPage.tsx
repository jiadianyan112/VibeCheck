...
function RemotePreviewSummary({ preview }: { preview: SubmissionDraftPreview }) {
  return (
    <section className="task-review-section stack" aria-label="提交预览">
...
function SubmittedVersion({ draft }: { draft: SubmissionDraft }) {
  ...
  return (
    <details className="task-review-section submission-version">
...
function PreviewSummary({ draft }: { draft: SubmissionDraft }) {
  ...
  return (
    <div className="submission-preview-grid">
      <section className="task-review-section stack" aria-labelledby="submission-detail-summary">
        <div><h2 id="submission-detail-summary">作品预览</h2></div>
        <dl className="submission-summary-grid">
          <div><dt>作品名称</dt><dd>{fields.currentName ?? '未填写'}</dd></div>
          <div><dt>一句话介绍</dt><dd>{fields.oneLineDefinition ?? '未填写'}</dd></div>
          <div><dt>公开地址</dt><dd>{fields.publicUrl ?? '未填写'}</dd></div>
          <div><dt>访问状态</dt><dd>{fields.accessStatus ? accessStatusText[fields.accessStatus] : '未填写'}</dd></div>
          {portfolio ? <>
            <div><dt>创作者身份</dt><dd>{list(fields.creatorRoles, creatorRoleLabels)}</dd></div>
            <div><dt>建站目的</dt><dd>{list(fields.primaryGoals, primaryGoalLabels)}</dd></div>
            <div><dt>核心内容</dt><dd>{list(fields.coreModules, coreModuleLabels)}</dd></div>
...
export function SubmissionReviewPage({ draft, coverUrl }: { draft: SubmissionDraft; coverUrl?: string }) {
  ...
  if (draft.status === 'draft') {
    if (remoteSubmission) {
      ...
      const retryRemoteError = previewError?.retryable
        ...
        : undefined
      return (
        <TaskShell currentStep="preview" aside={<TaskPreview><LivePreview fields={draft.fields} coverUrl={coverUrl} /></TaskPreview>} status={<StatusBeacon state={busy || previewLoading ? 'busy' : remotePreviewError || remoteSubmitError || error ? 'warning' : 'idle'} label={busy ? '正在提交审核' : previewLoading ? '正在生成提交预览' : remoteSubmission && !currentRemotePreview ? '等待有效预览' : '请核对提交内容'} />} title="发布预览" description="确认最新草稿、媒体和公开证据已准备就绪；只有有效预览才可以提交审核。">
          <div className="stack">
    ...
    return (
      <TaskShell currentStep="preview" aside={<TaskPreview><LivePreview fields={draft.fields} coverUrl={coverUrl} /></TaskPreview>} status={<StatusBeacon state={busy || previewLoading ? 'busy' : remotePreviewError || remoteSubmitError || error ? 'warning' : 'idle'} label={busy ? '正在提交审核' : previewLoading ? '正在生成提交预览' : remoteSubmission && !currentRemotePreview ? '等待有效预览' : '请核对提交内容'} />} title="发布预览" description="确认社区卡片、详情摘要与来源说明；只有点击确认提交后才会创建审核状态。">
        <div className="stack">
          <PreviewSummary draft={draft} />
  ...
  if (remoteSubmission) {
    ...
    return (
      <TaskShell currentStep="preview" status={<StatusBeacon state={draft.status === 'approved' ? 'success' : draft.status === 'rejected' ? 'error' : draft.status === 'changes_requested' ? 'warning' : draft.status === 'withdrawn' ? 'idle' : 'pending'} label={statusLabel} />} title={`审核状态：${statusLabel}`} description="这里显示提交回执与审核状态；暂无可靠的预计完成时间。">
        <div className="stack">
          ...
          <Link className="button button--primary" to="/projects">返回作品广场</Link>
        </div>
      </TaskShell>
    )
  }
  ...
  return (
    <TaskShell currentStep="preview" status={<StatusBeacon state={draft.status === 'approved' ? 'success' : draft.status === 'rejected' ? 'error' : draft.status === 'changes_requested' ? 'warning' : draft.status === 'withdrawn' ? 'idle' : 'pending'} label={statusLabel} />} title={`审核状态：${statusLabel}`} description="在这里查看审核结果、修改意见和补充材料。预计完成时间无法确认时不会显示倒计时。">
      <div className="stack">
