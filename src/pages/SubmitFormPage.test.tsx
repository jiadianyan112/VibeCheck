...
describe('new project multi-step submission form', () => {
  ...
  it('publishes a portfolio with six essential facts and no borrowed asset ownership', async () => {
    ...
    expect(screen.getByText('作者公开资产').closest('div')).toHaveTextContent('发布后可在“管理作品”中添加')
    expect(screen.queryByText('目标用户')).not.toBeInTheDocument()
    expect(screen.queryByText('核心问题')).not.toBeInTheDocument()
    expect(screen.queryByText('练习形式')).not.toBeInTheDocument()
