...
import { Button, ErrorPanel, Input, LoadingState, Tag, useToast } from '../components'
import { ErrorSummary, LivePreview, StatusBeacon, TaskPreview, TaskShell, useCoverPreview } from '../components/task'
...
const stepErrorTargets: Record<SubmissionFormStep, Record<string, string>> = {
  prefill: { currentName: 'submission-currentName', oneLineDefinition: 'submission-definition', screenshotUrl: 'submission-screenshotUrl', repositoryUrl: 'submission-repositoryUrl', accessStatus: 'submission-access-status' },
  definition: { creatorRoles: 'submission-creatorRoles', primaryGoals: 'submission-primaryGoals', targetUsers: 'submission-targetUsers', coreProblem: 'submission-core-problem', useScenarios: 'submission-useScenarios' },
  solution: { coreModules: 'submission-coreModules', mainInputs: 'submission-mainInputs', mainOutputs: 'submission-mainOutputs', coreFlow: 'submission-core-flow' },
  development: {},
}
      ...
      <div className="submission-choice-grid">
        {values.map((value) => (
          <label className="choice-card" key={value}>
            <input
              type="checkbox"
              id={value === values[0] ? fieldId : undefined}
...
function PortfolioDefinitionStep({ draft, update }: { draft: SubmissionDraft; update: <K extends keyof SubmissionProjectFields>(field: K, value: SubmissionProjectFields[K]) => void }) {
  return <div className="submission-step-fields stack"><section className="submission-guidance"><strong>只确认最重要的定位</strong><p>选择你的身份和建站目的。网站类型、视觉与交互等信息会在公开页面核验后补充，也可以发布后再维护。</p></section><CheckboxField fieldId="submission-creatorRoles" legend="作者身份" values={creatorRoles} selected={draft.fields.creatorRoles ?? []} labels={portfolioLabels} error={draft.validationErrors.creatorRoles} onChange={(value) => update('creatorRoles', value)} /><CheckboxField fieldId="submission-primaryGoals" legend="建站目的" values={primaryGoals} selected={draft.fields.primaryGoals ?? []} labels={portfolioLabels} error={draft.validationErrors.primaryGoals} onChange={(value) => update('primaryGoals', value)} /></div>
}
...
function PortfolioStructureStep({ draft, update }: { draft: SubmissionDraft; update: <K extends keyof SubmissionProjectFields>(field: K, value: SubmissionProjectFields[K]) => void }) {
  return <div className="submission-step-fields stack"><section className="submission-guidance"><strong>确认页面里已经存在的核心内容</strong><p>只选择访客可以实际看到的模块。结构、视觉、主题和响应式状态不要求你自行分类。</p></section><CheckboxField fieldId="submission-coreModules" legend="核心内容模块" values={coreModules} selected={draft.fields.coreModules ?? []} labels={portfolioLabels} error={draft.validationErrors.coreModules} onChange={(value) => update('coreModules', value)} /></div>
}
...
function PrefillStep({ draft, update }: { draft: SubmissionDraft; update: <K extends keyof SubmissionProjectFields>(field: K, value: SubmissionProjectFields[K]) => void }) {
  ...
  return (
    <div className="submission-step-fields stack">
      ...
      <div>
        <Input id="submission-currentName" label="作品名称" value={fields.currentName ?? ''} error={draft.validationErrors.currentName} onChange={(event) => update('currentName', event.target.value)} />
      ...
      <div>
        <Input id="submission-screenshotUrl" label="截图地址（可跳过）" value={fields.screenshotUrl ?? ''} error={draft.validationErrors.screenshotUrl} onChange={(event) => update('screenshotUrl', event.target.value || null)} />
        <OriginalValue label="截图" value={original.screenshotUrl} />
      </div>
      <div>
        <Input id="submission-repositoryUrl" label="代码仓库（可跳过）" value={fields.repositoryUrl ?? ''} error={draft.validationErrors.repositoryUrl} onChange={(event) => update('repositoryUrl', event.target.value || null)} />
...
function DefinitionStep({ draft, update }: { draft: SubmissionDraft; update: <K extends keyof SubmissionProjectFields>(field: K, value: SubmissionProjectFields[K]) => void }) {
  return (
    <div className="submission-step-fields stack">
      <CheckboxField fieldId="submission-targetUsers" legend="目标用户" values={targetUsers} selected={draft.fields.targetUsers ?? []} labels={targetUserLabels} error={draft.validationErrors.targetUsers} onChange={(value) => update('targetUsers', value)} />
      ...
      <CheckboxField fieldId="submission-useScenarios" legend="使用场景" values={useScenarios} selected={draft.fields.useScenarios ?? []} labels={scenarioLabels} error={draft.validationErrors.useScenarios} onChange={(value) => update('useScenarios', value)} />
    </div>
  )
}
...
function SolutionStep({ draft, update }: { draft: SubmissionDraft; update: <K extends keyof SubmissionProjectFields>(field: K, value: SubmissionProjectFields[K]) => void }) {
  ...
  return (
    <div className="submission-step-fields stack">
      <CheckboxField fieldId="submission-mainInputs" legend="主要输入" values={inputTypes} selected={draft.fields.mainInputs ?? []} labels={inputTypeLabels} error={draft.validationErrors.mainInputs} onChange={(value) => update('mainInputs', value)} />
      <CheckboxField fieldId="submission-mainOutputs" legend="主要输出" values={outputTypes} selected={draft.fields.mainOutputs ?? []} labels={outputTypeLabels} error={draft.validationErrors.mainOutputs} onChange={(value) => update('mainOutputs', value)} />
      ...
      <CheckboxField fieldId="submission-practiceFormats" legend="练习形式" values={practiceFormats} selected={draft.fields.practiceFormats ?? []} labels={practiceFormatLabels} optional onChange={(value) => update('practiceFormats', value)} />
      <CheckboxField fieldId="submission-feedbackMethods" legend="反馈方式" values={feedbackMethods} selected={draft.fields.feedbackMethods ?? []} labels={feedbackMethodLabels} optional onChange={(value) => update('feedbackMethods', value)} />
...
function DevelopmentStep({
  ...
  return (
    <div className="submission-step-fields stack">
      <CheckboxField fieldId="submission-aiCodingTools" legend="AI 编程工具" values={aiCodingTools} selected={draft.fields.aiCodingTools ?? []} labels={aiCodingToolLabels} optional onChange={(value) => update('aiCodingTools', value)} />
...
export function SubmitFormPage() {
  ...
  const coverPreviewUrl = useCoverPreview(localDraft && localDraft.status !== 'draft' && localDraft.status !== 'changes_requested' ? null : coverFile)
  ...
  if (!user) {
    ...
    return <TaskShell currentStep={step} title="发布编辑"><section className="submit-login-callout stack"><h2>请先登录</h2><Link className="button button--primary" to={`/auth?return_to=${returnPath}`}>登录并返回当前草稿</Link></section></TaskShell>
  }
  if (previewRequested && remoteDraft && !previewReady && loadingRemote) return <TaskShell currentStep={step} title="发布编辑"><LoadingState label="正在恢复已同步草稿" /></TaskShell>
  if (previewRequested && draft && (previewReady || !remoteDraft)) return <SubmissionReviewPage draft={draft} coverUrl={coverPreviewUrl} />
  if (loadingRemote && !draft) return <TaskShell currentStep={step} title="发布编辑"><LoadingState label="正在恢复已同步草稿" /></TaskShell>
  if (!draft) return <TaskShell currentStep={step} title="未找到发布草稿" description={loadError ?? '草稿可能不存在、属于其他身份或已经删除。'}><Link className="button" to="/submit">返回地址检查</Link></TaskShell>
  if (expired) return <TaskShell currentStep={step} title="草稿已过期" description="该已同步草稿已停止编辑，请重新检查公开地址后创建新的草稿。"><Link className="button button--primary" to={`/submit?resumeUrl=${encodeURIComponent(draft.fields.publicUrl ?? '')}`}>重新检查地址</Link></TaskShell>
  ...
  const goNext = () => {
    ...
    if (Object.keys(errors).length) {
      ...
      requestAnimationFrame(() => document.querySelector<HTMLElement>('.task-error-summary')?.focus())
      return
    }
  ...
  return (
    <TaskShell currentStep={step} title="发布新作品" description={draft.fields.categoryId === 'personal_site_portfolio' ? '确认作品名称、介绍、公开地址、创作者身份、建站目的和核心内容；其他信息可以发布后补充。' : '分步补充作品介绍、解决方案和开发信息，未完成的内容会自动保存。'}
      status={<StatusBeacon state={saving ? 'busy' : saveError || materialsFeedback ? 'warning' : 'idle'} label={saving ? '正在准备提交材料' : saveError || materialsFeedback ? '需要处理后继续' : '已保存到本机'} detail="准备提交材料时同步到账户" />}
      aside={<><TaskPreview><LivePreview fields={draft.fields} coverUrl={coverPreviewUrl} /></TaskPreview><div className="task-completion"><strong>填写完成度 {completion?.percent ?? 0}%</strong><progress aria-label="填写完成度" value={completion?.completed ?? 0} max={completion?.total ?? 10} /><p>内容会先保存到本机，准备提交材料时才会同步到账户。</p></div></>}
    >
        <section className="submission-form-panel stack" aria-labelledby="submission-step-heading">
          ...
          <ErrorSummary errors={Object.entries(draft.validationErrors).flatMap(([field, message]) => {
            const target = stepErrorTargets[step][field]
            return target && message ? [{ targetId: target, message }] : []
          })} />
          {body}
