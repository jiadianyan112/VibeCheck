...
const guidedStepLabels: Record<SubmissionFormStep, string> = {
  prefill: '基础信息',
  definition: '定位与用途',
  solution: '核心内容',
  development: '开发与资产',
...
function guidedTaskSteps(step: SubmissionFormStep): readonly TaskStepItem[] {
  const currentIndex = submissionFormSteps.indexOf(step)
  return [
    { id: 'address', label: '检查地址', state: 'complete' },
    ...submissionFormSteps.map((editingStep, editingIndex): TaskStepItem => ({
      id: editingStep,
      label: guidedStepLabels[editingStep],
      state: editingIndex < currentIndex ? 'complete' : editingStep === step ? 'current' : 'upcoming',
    })),
...
function CheckboxField<T extends string>({
  ...
  const errorId = useId()
  return (
    <fieldset id={id} className="submission-choice-field" tabIndex={-1} aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined}>
...
function PortfolioDefinitionStep({ draft, update }: { draft: SubmissionDraft; update: <K extends keyof SubmissionProjectFields>(field: K, value: SubmissionProjectFields[K]) => void }) {
  return <div className="submission-step-fields stack"><section className="submission-guidance"><strong>只确认最重要的定位</strong><p>选择你的身份和建站目的。网站类型、视觉与交互等信息会在公开页面核验后补充，也可以发布后再维护。</p></section><CheckboxField id="submission-creator-roles" legend="作者身份" values={creatorRoles} selected={draft.fields.creatorRoles ?? []} labels={portfolioLabels} error={draft.validationErrors.creatorRoles} onChange={(value) => update('creatorRoles', value)} /><CheckboxField id="submission-primary-goals" legend="建站目的" values={primaryGoals} selected={draft.fields.primaryGoals ?? []} labels={portfolioLabels} error={draft.validationErrors.primaryGoals} onChange={(value) => update('primaryGoals', value)} /></div>
}
...
function PortfolioStructureStep({ draft, update }: { draft: SubmissionDraft; update: <K extends keyof SubmissionProjectFields>(field: K, value: SubmissionProjectFields[K]) => void }) {
  return <div className="submission-step-fields stack"><section className="submission-guidance"><strong>确认页面里已经存在的核心内容</strong><p>只选择访客可以实际看到的模块。结构、视觉、主题和响应式状态不要求你自行分类。</p></section><CheckboxField id="submission-core-modules" legend="核心内容模块" values={coreModules} selected={draft.fields.coreModules ?? []} labels={portfolioLabels} error={draft.validationErrors.coreModules} onChange={(value) => update('coreModules', value)} /></div>
}
...
function PrefillStep({ draft, update }: { draft: SubmissionDraft; update: <K extends keyof SubmissionProjectFields>(field: K, value: SubmissionProjectFields[K]) => void }) {
  ...
  return (
    <div className="submission-step-fields stack">
      ...
      <div>
        <Input id="submission-current-name" label="作品名称" value={fields.currentName ?? ''} error={draft.validationErrors.currentName} onChange={(event) => update('currentName', event.target.value)} />
      ...
      <div>
        <Input id="submission-repository-url" label="代码仓库（可跳过）" value={fields.repositoryUrl ?? ''} error={draft.validationErrors.repositoryUrl} onChange={(event) => update('repositoryUrl', event.target.value || null)} />
...
function DefinitionStep({ draft, update }: { draft: SubmissionDraft; update: <K extends keyof SubmissionProjectFields>(field: K, value: SubmissionProjectFields[K]) => void }) {
  return (
    <div className="submission-step-fields stack">
      <CheckboxField id="submission-target-users" legend="目标用户" values={targetUsers} selected={draft.fields.targetUsers ?? []} labels={targetUserLabels} error={draft.validationErrors.targetUsers} onChange={(value) => update('targetUsers', value)} />
      ...
      <CheckboxField id="submission-use-scenarios" legend="使用场景" values={useScenarios} selected={draft.fields.useScenarios ?? []} labels={scenarioLabels} error={draft.validationErrors.useScenarios} onChange={(value) => update('useScenarios', value)} />
    </div>
  )
}
...
function SolutionStep({ draft, update }: { draft: SubmissionDraft; update: <K extends keyof SubmissionProjectFields>(field: K, value: SubmissionProjectFields[K]) => void }) {
  ...
  return (
    <div className="submission-step-fields stack">
      <CheckboxField id="submission-main-inputs" legend="主要输入" values={inputTypes} selected={draft.fields.mainInputs ?? []} labels={inputTypeLabels} error={draft.validationErrors.mainInputs} onChange={(value) => update('mainInputs', value)} />
      <CheckboxField id="submission-main-outputs" legend="主要输出" values={outputTypes} selected={draft.fields.mainOutputs ?? []} labels={outputTypeLabels} error={draft.validationErrors.mainOutputs} onChange={(value) => update('mainOutputs', value)} />
      ...
      <CheckboxField id="submission-practice-formats" legend="练习形式" values={practiceFormats} selected={draft.fields.practiceFormats ?? []} labels={practiceFormatLabels} optional onChange={(value) => update('practiceFormats', value)} />
      <CheckboxField id="submission-feedback-methods" legend="反馈方式" values={feedbackMethods} selected={draft.fields.feedbackMethods ?? []} labels={feedbackMethodLabels} optional onChange={(value) => update('feedbackMethods', value)} />
      <label className="field" htmlFor="submission-differentiation"><span className="field__label">差异化说明（可跳过）</span><textarea id="submission-differentiation" className="input textarea" value={draft.fields.differentiation ?? ''} onChange={(event) => update('differentiation', event.target.value)} /></label>
    </div>
  )
}
...
function DevelopmentStep({
  ...
  return (
    <div className="submission-step-fields stack">
      <CheckboxField id="submission-ai-coding-tools" legend="AI 编程工具" values={aiCodingTools} selected={draft.fields.aiCodingTools ?? []} labels={aiCodingToolLabels} optional onChange={(value) => update('aiCodingTools', value)} />
...
export function SubmitFormPage() {
  ...
  const statusDetail = saving
    ...
    : saveError?.status === 409
      ...
      : materialsFeedback
        ? '请在编辑区查看详细状态，并在适合时重试。'
        : '编辑内容已保存到本机，准备提交材料时才会同步到账户。'
  const selectTaskStep = (taskStep: TaskStepItem) => {
    if (taskStep.id === 'address') {
      navigate(`/submit?resumeUrl=${encodeURIComponent(draft.fields.publicUrl ?? '')}`)
      return
    }
    if (!submissionFormSteps.includes(taskStep.id as SubmissionFormStep)) return
    const nextStep = taskStep.id as SubmissionFormStep
    if (nextStep === step) return
    dispatch({ type: 'DRAFT_UPSERT', draft: { ...draft, step: nextStep, updatedAt: new Date().toISOString() } })
    navigate(`/submit/new?${new URLSearchParams({ draft: draft.id, step: nextStep })}`)
  }
  const livePreview = (
    <LivePreview
      ariaLabel="当前草稿预览"
      eyebrow="当前草稿"
      title="实时预览"
      description="这里直接反映当前填写内容，未保存前也会更新。"
    >
  ...
  return (
    <div className="highfi-scope submit-form-scope">
      <TaskShell
        eyebrow="发布作品"
        title="补充作品信息"
        description={draft.fields.categoryId === 'personal_site_portfolio' ? '确认作品名称、介绍、公开地址、创作者身份、建站目的和核心内容；其他信息可以发布后补充。' : '分步补充作品介绍、解决方案和开发信息，未完成的内容会自动保存。'}
        rail={<StepRail steps={guidedTaskSteps(step)} onStepSelect={selectTaskStep} ariaLabel="发布步骤" />}
        aside={<div className="submit-form-aside stack stack--small"><StatusBeacon tone={statusTone} label={statusLabel} detail={statusDetail} />{livePreview}</div>}
      >
        <section className="submit-form-panel stack" aria-labelledby="submission-step-heading">
          <header className="submit-form-panel__header">
            <div><p className="submit-form-panel__eyebrow">第 {index + 2} 步 / 6</p><h2 id="submission-step-heading">{guidedStepLabels[step]}</h2></div>
            <div className="cluster"><Tag tone="dashed">{draft.fields.categoryId === 'personal_site_portfolio' ? '个人主页与作品集' : 'AI 学习与题库'}</Tag><Tag tone="dashed">已保存到本机</Tag></div>
          </header>
          {errorSummaryItems.length > 0 ? <ErrorSummary errors={errorSummaryItems} /> : null}
          ...
          {body}
          <footer className="task-primary-actions cluster cluster--between"><Button type="button" onClick={goBack}>上一步</Button><Button type="button" variant="primary" loading={saving} onClick={goNext}>{index === submissionFormSteps.length - 1 ? finalActionLabel : '保存并继续'}</Button></footer>
        </section>
      </TaskShell>
    </div>
  )
