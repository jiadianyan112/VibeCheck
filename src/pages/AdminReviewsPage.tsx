...
export function AdminReviewsPage() {
  ...
  return (
    <div className="admin-page stack">
      ...
      <section className="wire-panel stack" aria-labelledby="restrict-heading"><div><h2 id="restrict-heading">限制已发布作品展示</h2></div><p>该动作仅管理员可用，不删除作品、来源或历史。</p><label className="field"><span className="field__label">选择作品</span><select className="input" value={restrictId} onChange={(event) => setRestrictId(event.target.value as typeof restrictId)}>{allProjects.map((project) => <option key={project.id} value={project.id}>{project.currentName.state === 'known' ? project.currentName.value : project.id}（{project.reviewStatus}）</option>)}</select></label><Button variant="danger" disabled={!isAdminWorkflowAllowed('display_restricted', state.session.role)} onClick={requestRestriction}>限制展示</Button></section>
